...
The maximum rate of change for the upper bounds is about -2.0, however, the bifurcation of the plot indicates that this is not achievable. ADD SOMETHING ABOUT NEGATIVE EMISSIONS?
...
plot(year[2:j], Teq[2:j], ylab="Equilibrium temperature (C)")
```
About .023 is the maximum rate of change for energy intensity in the lower bounds scenario to maintain warming under 2 degrees C.
...
plot(year[2:j], Teq[2:j], ylab="Equilibrium temperature (C)")
```
Approximately -1.9 is the max. rate of change of change. This indicates that this may not be achievable, SOMETHING ABOUT NEGATIVE EMISSIONS? 
...
#initial conditions
   ...
   bau_damages <- as.data.frame(climateDamage) #save bau annual damages as a vector
   write.csv(bau_damages,"BAU.csv", row.names = FALSE)
```
...
# initializing for first period    
    ...
    for (j in 2:p_years) {
      ...
      #emissions
      em_MtCO2[j]<-pop[j]*gdppc[j]*enInt[j]*emInt[j]+extra_em[j] #Kaya identity
      em_MtCO2[2]<-pop[j]*gdppc[j]*enInt[j]*emInt[j]+.000001 # add the bump from 2011
      em_ppm[j]<-em_MtCO2[j]/7810 # convert to atmospheric CO2 concentrations
   ...
   marginal_damages <- as.data.frame(climateDamage) #save extra damages as a vector 
   write.csv(marginal_damages,"marginal.csv", row.names = FALSE)
  
```

#### Calculating the social cost of carbon 

Under a 1.5% discount rate, the SCC is $252/ton of CO2. Under a 3% discount rate, the SCC is about $30.20/ton of CO2. The spread in values is due to the varying discount rates (DR). The lower DR places a higher value on future climate damages than the 3% DR, so that each year is associated with higher damage costs. These costs are higher at earlier time periods and accrue to a higher total value than the 3% rate.  
